...
/**
 * The {@link Var} class provides utility methods for converting between Bukkit API objects
 * and their corresponding NMS (Net Minecraft Server) counterparts, as well as handling
 * component conversions and unsafe casting. These methods often rely on reflection
 * due to version-specific NMS paths.
 */
public class Var
{
    /**
     * Converts a Bukkit {@link ItemStack} to its NMS (Net Minecraft Server) equivalent.
     * This method uses reflection to call the appropriate {@code asNMSCopy} method
     * from the CraftBukkit library, which varies by server version.
     *
    ...
    /**
     * Retrieves the NMS (Net Minecraft Server) equivalent of a Bukkit {@link World} object.
     * This typically involves calling the {@code getHandle()} method on the Bukkit World object
     * via reflection to access the underlying NMS world instance.
     *
    ...
    /**
     * Converts a plain text {@link String} into a {@link WrappedComponent} (NMS chat component equivalent).
     * This method uses reflection to call the appropriate {@code fromStringOrNull} method
     * from CraftBukkit's CraftChatMessage utility, which handles parsing text into components.
     *
    ...
    /**
     * Converts a {@link WrappedComponent} (NMS chat component equivalent) back into a plain text {@link String}.
     * This method uses reflection to call the appropriate {@code fromComponent} method
     * from CraftBukkit's CraftChatMessage utility.
     *
